    ...
    const updateTask=async(event)=>{
        ...
        try{
          ...
          if(data.status===200){
            ...
            setModalContent(
              `Record ID ${taskID} updated with record name ${taskName} and date ${taskDate}`
            );
            setModalVisible(true);
          }else{
            throw new Error("Record cannot be updated because of date clash")
          }

        }catch (error) {
          setModalContent("Record cannot be updated");
          setModalVisible(true);
        }
    }
    return(
